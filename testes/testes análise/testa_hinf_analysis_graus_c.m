% Script de Teste para Norma Hinf Contínua 
clear; clc; close all;

fprintf(' Iniciando Teste de Análise Hinf Contínua Incerta ');
...
for j=1:N
    ...
    D{j} = zeros(size(Di{j})); % D zerada conforme script original
end
...
if N == 2
    fig1 = figure('Name', 'Hinf Contínua: Teste 1', 'Color', 'w');
    hold on; grid on;
    title(sprintf('H_\infty Contínua: Variação de W (P grau 1) - Sistema %d', idx_sys));
...
for idx = 1:length(graus_teste1)
    ...
    out = hinf_lmi_c_incerto_param(A, B, C, D, opt);
...
if N == 2
    fig2 = figure('Name', 'Hinf Contínua: Teste 2', 'Color', 'w');
    hold on; grid on;
    title(sprintf('H_\infty Contínua: Variação de P (W grau 0) - Sistema %d', idx_sys));
    ylabel('Norma H_\infty'); xlabel('\alpha_1');
end
...
for idx = 1:length(graus_teste2)
    ...
    if out.feas == 1
        ...
        fprintf('Inviável ou Erro Numérico.\n');
    end
end
if N == 2
    legend('Location', 'best'); xlim([0 1]);
...
if N == 2
    fig3 = figure('Name', 'Hinf Contínua: Teste 3', 'Color', 'w');
    hold on; grid on;
    title(sprintf('H_\infty Contínua: Variação de W (P grau 2) - Sistema %d', idx_sys));
    ylabel('Norma H_\infty'); xlabel('\alpha_1');
end
...
if isempty(H1_table)
    ...
    disp(T1);
    writetable(T1, 'teste_Hinf_cont_Tabela1.csv');
end
...
if isempty(H2_table)
    ...
    disp(T2);
    writetable(T2, 'teste_Hinf_cont_Tabela2.csv');
end
...
if isempty(H3_table)
    ...
    disp(T3);
    writetable(T3, 'teste_Hinf_cont_Tabela3.csv');
end

% Salva os gráficos
if N == 2
    if ~isempty(H1_table), figure(fig1); print('teste_Hinf_cont_grafico1', '-depsc'); end
    if ~isempty(H2_table), figure(fig2); print('teste_Hinf_cont_grafico2', '-depsc'); end
    if ~isempty(H3_table), figure(fig3); print('teste_Hinf_cont_grafico3', '-depsc'); end
