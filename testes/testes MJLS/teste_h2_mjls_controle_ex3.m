...
fprintf(' Iniciando Teste de Análise H2 para MJLS Incerto - Exemplo 3 \n');
...
opt = struct(); 
opt.solver = 'mosek'; % Pode usar 'mosek' se preferir
...
for deg = graus_P_teste
    for idx = 1:length(graus_rho_teste)
        ...
        out =  h2_lmi_mjls_d_incerto_controle(A, B, E, Cz, D, Gamma, mu, opt);
        ...
        if out.feas == 1
            ...
            H_table = [H_table; deg, d, erro_norma, max_gap, out.V, out.L];
        else
            fprintf('Inviável.\n');
        end
...
if isempty(H_table)
    ...
    T_H = array2table(H_table, 'VariableNames', {'Grau_P', 'Grau_rho', 'Erro_Norma', 'Max_Gap', 'Variaveis', 'Linhas_LMI'});
    disp(T_H);
    
    writetable(T_H, 'teste_controle_H2_MJLS_Ex3_N2_Tabela.csv');
%     writetable(T_H, 'teste_controle_H2_MJLS_Ex3_N2_indep_Tabela.csv');
    fprintf('Tabela salva como teste_controle_H2_MJLS_Ex3_Tabela.csv\n');
end

if N == 2 && ~isempty(H_table)
    print('teste_controle_H2_MJLS_Ex3_N2_grafico', '-depsc'); 
    savefig('teste_controle_H2_MJLS_Ex3_N2_grafico.fig');
%     print('teste_controle_H2_MJLS_Ex3_N2_indep_grafico', '-depsc'); 
%     savefig('teste_controle_H2_MJLS_Ex3_N2_indep_grafico.fig');
