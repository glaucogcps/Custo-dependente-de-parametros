function output = h2_lmi_mjls_d_incerto_analise(A,E,Cz,Ez,Gamma,mu,varargin)
% function output = h2_lmi_mjls_d_incerto_analise(A,E,Cz,Ez,Gamma,mu,varargin)
%
% Evaluate the H-2 norm of a discrete-time MJLS with polytopic uncertainties. 
...
%   .h2        : H-2 norm guaranteed worst-case (0 if unfeasible)
%   .wc        : Pior custo H-2 garantido encontrado no grid
%   .normSysWc : Custo H-2 real do sistema no pior caso
...
%
% Date: 25/03/2026
% Author: cfmorais@unicamp.br, glauco.gcps@gmail.com

if nargin < 6
    error('h2_mjls_incerto:NotEnoughInputs', 'São necessários A, E, Cz, Ez, Gamma e mu.');
end
...
for i = 1:length(input_cells)
    if ~iscell(input_cells{i})
        error('h2_mjls_incerto:InvalidInputType', 'O argumento %s deve ser um cell array.', input_names{i});
    end
    if numel(input_cells{i}) ~= num_modos
        error('h2_mjls_incerto:DimensionMismatch', 'Todos os cell arrays do sistema devem ter o mesmo número de modos (%d).', num_modos);
    end
end

if ~iscell(Gamma)
    error('h2_mjls_incerto:InvalidGammaType', 'Gamma deve ser um cell array contendo os vértices do politopo de transição.');
end
...
if dims_A(1) ~= dims_A(2)
    error('h2_mjls_incerto:InvalidADim', 'Matrizes A devem ser quadradas (nx x nx).');
end
if dims_E(1) ~= nx
    error('h2_mjls_incerto:IncompatibleDims', 'O número de linhas de E deve ser igual ao de A (nx = %d).', nx);
end
if dims_Cz(2) ~= nx
    error('h2_mjls_incerto:IncompatibleDims', 'O número de colunas de Cz deve ser igual ao de A (nx = %d).', nx);
end
if dims_Ez(1) ~= nz || dims_Ez(2) ~= nw
    error('h2_mjls_incerto:IncompatibleDims', 'Dimensões de Ez devem ser (nz x nw) -> (%d x %d).', nz, nw);
end
...
for k = 1:num_modos
    if ~isnumeric(A{k}) || ~isnumeric(E{k}) || ~isnumeric(Cz{k}) || ~isnumeric(Ez{k})
        error('h2_mjls_incerto:NonNumeric', 'O conteúdo de todas as células no modo %d deve ser numérico.', k);
    end
    if ~isequal(size(A{k}), dims_A) || ~isequal(size(E{k}), dims_E) || ...
       ~isequal(size(Cz{k}), dims_Cz) || ~isequal(size(Ez{k}), dims_Ez)
        error('h2_mjls_incerto:VaryingDims', 'As dimensões das matrizes não podem variar entre os modos (verifique o modo %d).', k);
    end
end
...
for r = 1:num_vertices
    if size(Gamma{r}, 1) ~= num_modos || size(Gamma{r}, 2) ~= num_modos
        error('h2_mjls_incerto:InvalidGamma', 'Cada vértice de Gamma deve ser quadrado com dimensão igual ao número de modos (%d x %d).', num_modos, num_modos);
    end
    if any(Gamma{r}(:) < 0) || any(abs(sum(Gamma{r}, 2) - 1) > 1e-6)
        error('h2_mjls_incerto:InvalidGammaProb', 'O vértice %d de Gamma não é uma matriz estocástica válida.', r);
    end
end

if length(mu) ~= num_modos
    error('h2_mjls_incerto:InvalidMu', 'O vetor mu deve ter comprimento igual ao número de modos (%d).', num_modos);
end
if any(mu < 0) || abs(sum(mu) - 1) > 1e-6
    error('h2_mjls_incerto:InvalidMuProb', 'mu deve ser uma distribuição de probabilidade válida (elementos >= 0 e soma = 1).');
end
...
if nargin > 6
    if nargin == 7 && isstruct(varargin{1})
        ...
        try
            ...
            error('h2_mjls_incerto:InvalidOptions', 'Opções inválidas. Use struct ou pares nome-valor.');
        end
    end
end
...
if options.h2 > 0
    % Apenas checagem de viabilidade para um H-2 dado
    rho_alpha = options.h2 * options.h2;
    options.tolerance = 0;
    obj = [];
else
    ...
    if options.op == 1
        ...
        rho = sdpvar(1, 1);
        rho_alpha = rolmipvar(rho, 'rho', N, 0); % Converte para rolmipvar grau 0
        obj = rho;
    else        
...
for i=1:sigma
    %trace condition     
    soma = soma + mu(i)*trace(E{i}'*Lambda{i}*Xcal*E{i}+Ez{i}'*Ez{i});
    %grammian condition 
    LMIs = [LMIs, A{i}'*Lambda{i}*Xcal*A{i} - P{i} + Cz{i}'*Cz{i} <= 0];
end 
...
%         out_nominal = h2_lmi_mjls_d(A, E, Cz, Ez, Gamma_aux, mu);
%         
        [h2_real, feas_real] = calc_h2_nominal_kronecker(A, E, Cz, Ez, Gamma_aux, mu);
...
%     out_wc = h2_lmi_mjls_d(A, E, Cz, Ez, Gamma_wc, mu);
%     if out_wc.feas == 1
%         output.normSysWc = out_wc.h2;
         
    % Calcula a norma real do pior caso garantido via Kronecker
    [h2_wc_real, feas_wc_real] = calc_h2_nominal_kronecker(A, E, Cz, Ez, Gamma_wc, mu);
    if feas_wc_real == 1
...
function [h2_norm, feas] = calc_h2_nominal_kronecker(A, E, C, Ez, Gamma, mu)
...
% P_i - A_i * sum(Gamma_ji * P_j) * A_i^T = mu_i * E_i * E_i^T
    ...
    % Calcula o custo H2 final: sum_{i=1}^sigma trace(C_i * S_i * C_i^T) + energia estática (Ez)
    h2_sq = 0;
    for i = 1:sigma
        % Custo dinâmico (estados) + Custo estático (transmissão direta Dzw)
        h2_sq = h2_sq + trace(C{i} * S{i} * C{i}') + mu(i) * trace(Ez{i}' * Ez{i});
    end
