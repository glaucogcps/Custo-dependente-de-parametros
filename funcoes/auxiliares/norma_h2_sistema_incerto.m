function output = norma_h2_sistema_incerto(A_cell, B_cell, C_cell, D_cell, type_analysis, varargin)
% norma_h2_sistema_incerto Calcula a norma H2 de um sistema incerto
% para sistemas contínuos ou discretos.
%
% Esta função atua como um dispatcher, chamando a função apropriada
% (norma_h2_sistema_incerto_continuous ou norma_h2_sistema_incerto_discrete)
% com base no 'type_analysis' fornecido.
%
% Sintaxe:
%   output = norma_h2_sistema_incerto(A_cell, B_cell, C_cell, D_cell, type_analysis, optional_args)
%
% Entradas:
...
%                   - Para 'continuous': .max_h2_norm, .alpha_star
%                   - Para 'discrete':   .max_h2_norm, .alpha_star
% Date: 25/09/2025
...
if nargin < 5 % A_cell, B_cell, C_cell, D_cell, type_analysis
    error('norma_h2_sistema_incerto:NotEnoughInputs', ...
...
if ~ischar(type_analysis)
    error('norma_h2_sistema_incerto:InvalidTypeArgument', ...
...
if strcmp(type_analysis_lower, 'continuous') || strcmp(type_analysis_lower, 'c')
    fprintf('Chamando norma_h2_sistema_incerto_continuous para análise de sistema contínuo.\n');
    output = norma_h2_sistema_incerto_continuous(A_cell, B_cell, C_cell, D_cell, varargin{:}); % Usa {:} para desempacotar varargin
elseif strcmp(type_analysis_lower, 'discrete') || strcmp(type_analysis_lower, 'd')
    fprintf('Chamando norma_h2_sistema_incerto_discrete para análise de sistema discreto.\n');
    output = norma_h2_sistema_incerto_discrete(A_cell, B_cell, C_cell, D_cell, varargin{:}); % Usa {:} para desempacotar varargin
else
    error('norma_h2_sistema_incerto:UnknownAnalysisType', ...
